...
#-----------roll------------#
if ($setting['roll_Status'] == "✅ تایید قانون روشن است" && $user['roll_Status'] == 0 && $text != "✅ تایید موارد" && !in_array($from_id, $admin_ids)) {
...
✅ برای تایید تمدید اشتراک روی دکمه زیر کلیک کنید.
💲 جهت تمدید ابتدا حساب خود را شارژ کنید.";
...
لوکیشن اشتراک کاربر : {$nameloc['Service_location']}";
    ...
    $username = $dataget[1];
    update("user", "Processing_value", $username, "id", $from_id);
    $textextra = "🐳 مقدار حجم مورد نیاز را ارسال کنید.

⚡️قیمت هر گیگ حجم اضافه  {$setting['Extra_volume']} است.";
...
💰 قیمت هر گیگ حجم اضافه :  {$setting['Extra_volume']} تومان
📝 مبلغ  فاکتور شما :  $priceextra تومان
📥 حجم اضافه درخواستی : $text  گیگابایت

✅ جهت پرداخت و افزایش حجم اشتراک، روی دکمه زیر کلیک کنید.";
...
——————————————————
 ...
 ⚠️ جهت تکمیل عملیات، حتمااا و حتما یک اسکرین شات از رسید پرداخت، همینجا داخل ربات ارسال کنید.🙏🏻";
...
با استفاده از لینک اختصاصی که ربات به شما میدهد میتوانید دوستان و آشنایان خود را به ربات دعوت کنید و با خرید هر زیرمجموعه به شما پورسانت داده خواهد شد و میتوانید از این طریق کسب درآمد خود را داشته باشید.🤑

همچنین با قابلیت زیرمجموعه گیری شما میتوانید بدون نیاز به هیچ پرداختی حساب کاربری خود را شارژ کنید و از خدمات ربات استفاده کنید.🎉

🎁 مبلغ هدیه به ازای هر عضویت :  $price_Discount
💵 میزان پورسانت از خرید هر زیرمجموعه :  $affiliatespercentage";
...
if (preg_match('/Confirm_pay_(\w+)/', $datain, $dataget)) {
    ...
    $text_report = "📣 یک ادمین رسید پرداخت درگاه را تایید کرد.
